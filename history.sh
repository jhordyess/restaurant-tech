...
yarn add -D gh-pages cname-webpack-plugin && npm pkg set scripts.predeploy="npm run lint && npm run build" && npm pkg set scripts.deploy="gh-pages -d dist"
...
yarn add -D prettier prettier-plugin-tailwindcss && touch .prettierrc.json && npm pkg set scripts.format="prettier --write ."

# Linter with ESLint
yarn add -D eslint && yarn eslint --init && npm pkg set scripts.lint="eslint . --ext .ts,.tsx"

# Local server with serve
yarn add -D serve && npm pkg set scripts.start="npm run build && serve -s dist -l 3000"

# Git hooks with Husky
yarn add -D husky && yarn husky install && yarn husky add .husky/pre-push "npm run lint" && npm pkg set scripts.prepare="husky install"
